...
const run = async () => {
  console.log(`🔄 Seeding user profiles for ${users.length} users...`)
  ...
  await writer.close()
  console.log(`✅ Completed. User profiles written: ${written}`)
  process.exit(0)
}

run().catch((err) => {
  console.error('❌ Failed seeding user profiles', err)
  process.exit(1)
})
